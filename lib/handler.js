...
function checkOwner(sender) {
  try {
    ...
    const resultado = owners.some(entry => {
      ...
      console.log(chalk.yellow(`🔍 comparando owner: ${ownerStr} -> ${ownerLimpio}`));
      return senderLimpio === ownerLimpio;
    });

    console.log(chalk.yellow(`🔍 resultado owner: ${resultado ? 'ES OWNER' : 'NO ES OWNER'}`));
    return resultado;
  } catch (e) {
...
function dfail(tipo, m, conn) {
  ...
  // Obtenemos el texto directamente de global.msj
  ...
  try {
    ...
    if (isGroup) {
      try {
        ...
        // 🔧 FIX: Normalizar JIDs para comparación robusta
        ...
        const userParticipant = participants.find(p => {
          ...
          return pNormalized === senderNormalized || areJidsSameUser(p.id, sender);
        });
        ...
        const botParticipant = participants.find(p => {
          const pNormalized = normalizeJid(p.id);
          return pNormalized === botNormalized || areJidsSameUser(p.id, botJid);
        });
    ...
    for (const filePath of archivos) {
      try {
        ...
        if (plugin.owner && !isOwner) {
          console.log(chalk.red('❌ Bloqueado: requiere owner'));
          dfail('owner', m, conn);
          return;
        }

        if (plugin.group && !isGroup) {
          console.log(chalk.red('❌ Bloqueado: requiere grupo'));
          dfail('group', m, conn);
          return;
        }

        if (plugin.private && isGroup) {
          console.log(chalk.red('❌ Bloqueado: requiere privado'));
          dfail('private', m, conn);
          return;
        }

        if (plugin.admin && !isAdmin) {
          console.log(chalk.red('❌ Bloqueado: requiere admin'));
          dfail('admin', m, conn);
          return;
        }

        if (plugin.botAdmin && !isBotAdmin) {
          console.log(chalk.red('❌ Bloqueado: requiere bot admin'));
        ...
        try {
          ...
          // Usar mensaje de error de global.msj
