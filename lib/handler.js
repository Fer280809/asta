  ...
  try {
    ...
    if (isGroup) {
      try {
        ...
        // 🔧 FIX: Obtener el JID base del bot (sin :X)
        const botJidRaw = conn.user?.jid || conn.user?.id;
        // Convertir 50373513267:18@s.whatsapp.net -> 50373513267@s.whatsapp.net
        const botJidBase = botJidRaw.replace(/:\d+/, '');
        // También crear versión @lid por si acaso
        const botNum = botJidBase.split('@')[0];
        const botJidLid = `${botNum}@lid`;
        
        console.log(chalk.cyan('📊 PARTICIPANTES DEL GRUPO:'));
        participants.forEach(p => {
          console.log(chalk.gray(`  - ${p.id}: ${p.admin || 'member'}`));
        });
        console.log(chalk.cyan(`🤖 Bot JID Raw: ${botJidRaw}`));
        console.log(chalk.cyan(`🤖 Bot JID Base: ${botJidBase}`));
        console.log(chalk.cyan(`🤖 Bot JID Lid: ${botJidLid}`));
        ...
        // 🔧 FIX CRÍTICO: Buscar bot por múltiples formatos de JID
        const botParticipant = participants.find(p => {
          const pBase = p.id.replace(/:\d+/, ''); // Quitar :X si existe
          return areJidsSameUser(p.id, botJidRaw) || 
                 areJidsSameUser(p.id, botJidBase) ||
                 pBase === botJidBase ||
                 p.id === botJidLid ||
                 pBase === botJidLid;
        });
        ...
        console.log(chalk.gray(`  Usuario: ${sender} | Admin: ${isAdmin}`));
        console.log(chalk.gray(`  Bot encontrado: ${botParticipant ? botParticipant.id : 'NO'} | Admin: ${isBotAdmin}`));
