...
async function dfail(tipo, m, conn) {
  ...
  try {
    ...
    try {
      ...
      console.error('Error enviando mensaje dfail:', e2);
    }
  }
}
  ...
  try {
    ...
    if (isGroup) {
      try {
        ...
        // Obtener JID del bot de múltiples fuentes
        ...
        console.log(chalk.cyan(`🤖 Bot JID: ${botJid}`));

        // 🔧 FIX: Buscar por areJidsSameUser que compara JIDs correctamente
        // o por número si falla
        ...
        // 🔧 FIX CRÍTICO: Buscar bot usando areJidsSameUser primero
        let botParticipant = participants.find(p => areJidsSameUser(p.id, botJid));
        
        // Si no lo encuentra, buscar por número
        if (!botParticipant) {
        ...
        console.log(chalk.gray(`  Bot: ${botJid} | Encontrado: ${botParticipant ? botParticipant.id : 'NO'} | Admin: ${isBotAdmin}`));
    ...
    for (const filePath of archivos) {
      try {
        ...
        console.log(chalk.magenta('🔐 PERMISOS:'), {
          owner: plugin.owner, group: plugin.group, admin: plugin.admin, botAdmin: plugin.botAdmin,
