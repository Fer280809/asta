...
function esMismoUsuario(jid1, jid2) {
  return limpiarNumero(jid1) === limpiarNumero(jid2)
}
...
function checkOwner(sender) {
  try {
    ...
    return owners.some(entry => {
      ...
      return limpiarNumero(ownerRaw) === senderNum
    })
  } catch (e) {
  ...
  try {
    ...
    if (isGroup) {
      try {
        ...
        // Obtener número del bot (solo números)
        const botNum = limpiarNumero(conn.user?.jid || conn.user?.id || '')
        
        console.log(chalk.cyan('📊 PARTICIPANTES DEL GRUPO:'))
        participants.forEach(p => {
          const num = limpiarNumero(p.id)
          console.log(chalk.gray(`  - ${num}: ${p.admin || 'member'}`))
        })
        console.log(chalk.cyan(`🤖 Bot Número: ${botNum}`))

        // Buscar usuario por número limpio
        const userParticipant = participants.find(p => {
          return esMismoUsuario(p.id, sender)
        })
        
        isAdmin = !!(userParticipant?.admin === 'admin' || userParticipant?.admin === 'superadmin')

        // Buscar bot por número limpio
        const botParticipant = participants.find(p => {
          return limpiarNumero(p.id) === botNum
        })
        ...
        console.log(chalk.gray(`  Bot encontrado: ${botParticipant ? limpiarNumero(botParticipant.id) : 'NO'} | Admin: ${isBotAdmin}`))
