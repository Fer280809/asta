  ...
  try {
    ...
    if (isGroup) {
      try {
        ...
        // 🔧 FIX: Buscar si el bot está en el grupo
        const botParticipant = participants.find(p => {
          const pNum = getPhoneNumber(p.id);
          return pNum === botNum;
        });
        
        botInGroup = !!botParticipant;
        ...
        console.log(chalk.gray(`  Bot en grupo: ${botInGroup ? 'SÍ' : 'NO'}`));
    ...
    for (const filePath of archivos) {
      try {
        ...
        // 🔧 FIX: Si el bot no está en el grupo, avisar específicamente
        if (plugin.botAdmin && !botInGroup) {
          console.log(chalk.red('❌ El bot no está en este grupo'));
          await dfail('botNotInGroup', m, conn);
          return;
        }
