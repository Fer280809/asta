  ...
  try {
    ...
    if (isGroup) {
      try {
        ...
        const botNum = extraerNumero(conn.user?.jid || conn.user?.id || '')
        ...
        console.log(chalk.cyan(`👤 Usuario Número: ${senderNum}`))

        // Buscar usuario por NÚMERO (no por JID)
        ...
        // Buscar bot por NÚMERO (no por JID)
